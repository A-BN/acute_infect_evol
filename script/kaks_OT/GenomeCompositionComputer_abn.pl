...
print "GFF: $gff3file \n";
print "observed mutations: $observed \n";
print "mask: $maskfile \n";
...
# $line = <GENELIST>;
# $line = <GENELIST>;
# I supressed the HEADER (lines starting with ##) manually :(
while( $line = <GENELIST>)
{
    print "$line";
    chomp $line;	# drop \n at end of line
...
my @Propora = pairwise {$a / $b} @Synonymous, @NonSynonymous;
my $PropMean = ze_mean(@Propora);
print "@Propora\n";
print "On average: $PropMean";
print "\n---\n ";
...
open (GENELIST, "<$observed");
# HEADER = S_AT_CG S_AT_GC S_AT_TA S_CG_GC S_CG_TA S_GC_TA NS_AT_CG NS_AT_GC NS_AT_TA NS_CG_GC NS_CG_TA NS_GC_TA
# $line = <GENELIST>; # skip header
$line = <GENELIST>;
print "\nmutations: \n$line\n";
chomp $line;    # drop \n at end of line
@b=split(/\t/,$line);
...
#if ($observedS == 0) {
  print "GLOU";
  my $invKaKs=1/($observedS/$expectedS);
  #my $invKaKs=$expectedS/$observedS;
  print "From inverse approach (i.e: NS as ref)\nKA/KS=  \n $invKaKs\n";
#}